...
def extract_mention_reviews(text):
    x = text.lower()
    x = nlp(x)
    try:
        matcher(x)
...
def create_matcher(x):
    global cnt
    x = re.split(r'[,|\s]*', i)
    if len(x) == 1:
        # print x
        cnt += 1
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={}, specs=[[{spacy.attrs.LOWER: x[0]}]],
                    on_match=merge_phrases)
        cnt += 1
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={},
                    specs=[[{spacy.attrs.LOWER: x[0]}, {spacy.attrs.IS_PUNCT: True}]], on_match=merge_phrases)

    elif len(x) == 2:
        cnt += 1
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={},
                    specs=[[{spacy.attrs.LOWER: x[0]}, {spacy.attrs.LOWER: x[1]}]], on_match=merge_phrases)
        cnt += 1
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={},
                    specs=[[{spacy.attrs.LOWER: x[0]}, {spacy.attrs.LOWER: x[1]}, {spacy.attrs.IS_PUNCT: True}]],
                    on_match=merge_phrases)
    elif len(x) == 3:
        cnt += 1
        # print x
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={},
                    specs=[[{spacy.attrs.LOWER: x[0]}, {spacy.attrs.LOWER: x[1]}, {spacy.attrs.LOWER: x[2]}]],
                    on_match=merge_phrases)
        cnt += 1
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={}, specs=[
            [{spacy.attrs.LOWER: x[0]}, {spacy.attrs.LOWER: x[1]}, {spacy.attrs.LOWER: x[2]},
        ...
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={}, specs=[
            [{spacy.attrs.LOWER: x[0]}, {spacy.attrs.LOWER: x[1]}, {spacy.attrs.LOWER: x[2]},
             {spacy.attrs.LOWER: x[3]}]], on_match=merge_phrases)
        cnt += 1
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={}, specs=[
            [{spacy.attrs.LOWER: x[0]}, {spacy.attrs.LOWER: x[1]}, {spacy.attrs.LOWER: x[2]}, {spacy.attrs.LOWER: x[3]},
             {spacy.attrs.IS_PUNCT: True}]], on_match=merge_phrases)
        ...
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={}, specs=[
            [{spacy.attrs.LOWER: x[0]}, {spacy.attrs.LOWER: x[1]}, {spacy.attrs.LOWER: x[2]}, {spacy.attrs.LOWER: x[3]},
             {spacy.attrs.LOWER: x[4]}]], on_match=merge_phrases)
    elif len(x) == 6:
        cnt += 1
        # print x
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={}, specs=[
            [{spacy.attrs.LOWER: x[0]}, {spacy.attrs.LOWER: x[1]}, {spacy.attrs.LOWER: x[2]}, {spacy.attrs.LOWER: x[3]},
             {spacy.attrs.LOWER: x[4]}, {spacy.attrs.LOWER: x[5]}]], on_match=merge_phrases)
    elif len(x) == 7:
        cnt += 1
        # print x
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={}, specs=[
        ...
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={}, specs=[
            [{spacy.attrs.LOWER: x[0]}, {spacy.attrs.LOWER: x[1]}, {spacy.attrs.LOWER: x[2]}, {spacy.attrs.LOWER: x[3]}, \
             {spacy.attrs.LOWER: x[4]}, {spacy.attrs.LOWER: x[5]}, {spacy.attrs.LOWER: x[6]},
             {spacy.attrs.LOWER: x[7]}]], on_match=merge_phrases)
    elif len(x) == 9:
        cnt += 1
        # print x
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={}, specs=[
            [{spacy.attrs.LOWER: x[0]}, {spacy.attrs.LOWER: x[1]}, {spacy.attrs.LOWER: x[2]}, {spacy.attrs.LOWER: x[3]}, \
             {spacy.attrs.LOWER: x[4]}, {spacy.attrs.LOWER: x[5]}, {spacy.attrs.LOWER: x[6]}, {spacy.attrs.LOWER: x[7]},
             {spacy.attrs.LOWER: x[8]}]], on_match=merge_phrases)
    elif len(x) >= 10:
        cnt += 1
        # print x
        matcher.add(entity_key=str(cnt), label='FOOD', attrs={}, specs=[
...
with open("mention.txt", "w") as mention:
    with open("reviews100000.json") as reviews:
        for cnt, i in enumerate(reviews.readlines()):
            ...
            mention_text = extract_mention_reviews(y)
            if mention_text != "":
                mention.write(
                    x["business_id"] + "~" + x["review_id"] + "~" + y + "~" + mention_text + "~" + str(x["stars"]))
                mention.write("\n")
